...
What each file is and what it owns. The README is for people who put komorebi on a page; this file is for people
who work on komorebi. The spec ([`komorebi-spec.md`](komorebi-spec.md)) holds the vision, the physics and the
model. Read the spec before you change what the engine does.
...
- **`examples/`** — the embeds the README points at: a page background, a panel, and the ES-module route. They ship with the site, so they are also the demos.
...
- **`embed-build.mjs`** — both deploy bundles, so there is one definition of a build. With no arguments (`nix run .#build`) it keeps every shipped look and writes `dist/komorebi.player.min.js`; with look names (`nix run .#embed -- '<look>'...`) it keeps those looks and writes `dist/komorebi.embed.min.js`. It asks the camera registry which cameras the kept looks select and prunes the rest, with their GLSL, uniform groups and per-frame uploads. It also removes the comments from the shader literals and folds the editor tier off. Check a bundle with `nix run .#embed-check` before you ship it.
...
- **`.github/workflows/pages.yml`** — the deploy: builds `dist/` and publishes the repo to GitHub Pages (komorebi.eljojo.net) on a push to `main`. The bundle is there to download; the README tells embedders to serve their own copy.
