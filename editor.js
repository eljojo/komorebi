...
const PANEL = [
  ...
  ['sel','receiver','receiver',[['floor',0],['curtain',1],['tent',2]],''],   // curtain handoff: the surface the light lands on — floor reflects, curtain transmits (moss velvet), tent stands that same cloth around you
  ['s','curtain_tt','curtain Tt',0,1,0.01,''],
  ['s','curtain_tint_r','moss R',0,1,0.01,''],
  ['s','curtain_tint_g','moss G',0,1,0.01,''],
  ['s','curtain_tint_b','moss B',0,1,0.01,''],
  ['s','curtain_distance_m','curtain dist',-20,20,0.1,''],
  ['s','fold_depth','drape',0,2,0.01,''],
  ['s','fold_scale','pleats',0.3,12,0.1,''],
  ['s','fold_coarsen','pleat coarsen',0,2,0.02,''],
  ['s','fold_warp','pleat depth (m)',0,0.3,0.005,''],   // GEOMETRY (§4.9): displaces the cloth, so the cast bends — 'drape' above only shades it
  ['s','velvet_sheen','velvet sheen',0,1.5,0.01,''],
  ['s','curtain_scatter','pile scatter',0,1,0.01,''],
  ['s','curtain_diffuse','glow bleed',0,1,0.01,''],       // THE GATE (§4.9): >0 routes the frame through linear HDR + two blur passes + a compositing tone-map
  ['s','curtain_diffuse_m','bleed (m)',0,0.2,0.005,''],   // how far the weave carries the light, in cloth metres
  ['s','mullion_tau','window bars',0,6,0.05,''],   // the gate: 0 = no window grid (analytic cloth-space occluder, curtain only)
  ['s','mullion_pitch_m','pane (m)',0.05,2,0.01,''],
  ['s','mullion_bar_m','bar (m)',0.002,0.15,0.001,''],
  ['s','mullion_depth_m','bar depth (m)',0,0.5,0.005,''],
  ['s','window_w_m','window w (m)',0,8,0.05,''],   // the gate: w×h = 0 = no aperture, the light runs to infinity (every look before this one)
  ['s','window_h_m','window h (m)',0,8,0.05,''],
  ['s','window_cx_m','window x',-10,10,0.05,''],
  ['s','window_cy_m','window y',-10,10,0.05,''],
  ['s','window_wall','wall leak',0,0.5,0.005,''],
  ['s','tent_ridge_h_m','crown h (m)',0.4,3,0.05,''],     // the enclosure's polytope (receiver 'tent'): crown plane, arch run, crown flat, length, end lean, the foot hip cap
  ['s','tent_half_w_m','half width (m)',0.4,3,0.05,''],
  ['s','tent_crown_w_m','crown w (m)',0,1.5,0.01,''],     // 0 = no flat top, the walls meet in a ridge line (the v1 A-frame)
  ['s','tent_shoulder_h_m','arch h (m)',0.1,2.5,0.01,''],   // the arch's widest point; clamped into [0.30,0.70]x crown height, outside which the profile doubles back
  ['s','tent_shoulder_w_m','arch bulge (m)',0.2,3,0.01,''], // clamped UP to the straight base->crown line: below it the arc curves the wrong way and would slice the crown off
  ['s','tent_len_m','length (m)',0.8,8,0.1,''],
  ['s','tent_end_lean','end lean',0,2,0.02,''],
  ['s','tent_end_apex_h_m','vent apex (m)',0.05,2,0.01,''],   // the top of the foot triangle, and where the two hips converge
  ['s','tent_hip_rake','hip rake',0.15,2,0.01,''],            // how much faster the hips rake back than the end wall; floored so they never coincide with it
  ['s','tent_eye_h_m','eye (m)',0,2,0.05,''],
  ['s','tent_fade','depth fade',0,0.4,0.005,''],
  ['s','tent_seam','ridge seam',0,6,0.05,''],   // the gate: 0 = no seam
  ['s','tent_mesh','mesh band',0,1,0.01,''],    // per-panel MATERIAL: the sides above the shoulder hem turn to mesh
  ...
  ['s','sway_pitch','3D pitch',0,2,0.05,''],
...
const TIPS = {
  ...
  curtain_tt:"<b>How much light the curtain lets through (its brightness).</b> Low = a thick dark velvet that only the hottest dapples punch through; high = a thin sheer the sun floods. Brightness only — the colour is set separately below, so a dark green velvet stays dark instead of glowing.",
  curtain_tint_r:"<b>How much RED the curtain dye passes.</b> Keep it low for moss — the dye drinks red, leaving the green. Only the hue matters here; the overall brightness is the Tt knob above.",
  curtain_tint_g:"<b>How much GREEN the curtain dye passes.</b> High for moss velvet — green is what leaks through, so the body of the curtain glows green-gold against the sunset.",
  curtain_tint_b:"<b>How much BLUE the curtain dye passes.</b> Low for moss. Nudge it for a cooler or warmer green.",
  curtain_distance_m:"<b>How far the curtain hangs from the tree.</b> The curtain is a standing wall; this slides it nearer or further along the ground, which — together with the sun's angle — sets where the tree's shadow lands on the cloth. Nudge it (with the sun angle and the pan) to frame the shadow on the curtain.",
  fold_depth:"<b>How draped the curtain is.</b> 0 = a flat sheet; up = the cloth hangs in deep vertical pleats. The pleats darken where the fabric turns edge-on (so more velvet to shine through) — the bright/dark folds of a real hanging curtain. This knob only <i>shades</i> the folds, though — <i>pleat depth</i> below is the one that pushes the cloth out into the room and bends the shadow over it.",
  fold_scale:"<b>How many pleats.</b> Low = a few broad waves; high = many tight gathers. Like choosing how full the curtain is bunched on its rod.",
  fold_warp:"<b>How far the pleats stand out of the cloth — and so how much the shadow bends over them.</b> The two fold knobs are different things and it's worth knowing which is which: <i>drape</i> only paints the folds on (dark where the cloth turns edge-on), while this one actually pushes the fabric out toward the room. That matters because a pleat sticking out catches the light a little earlier than the flat cloth beside it, so the leaf shadows and the window bars S-bend as they cross each fold instead of running dead straight over a picture of one. 0 = a flat sheet; a few centimetres is a real curtain.",
  fold_coarsen:"<b>How the pleats widen toward the bottom.</b> A heavy curtain has tight gathers at the rod that merge into broad waves at the hem (gravity). 0 = even pleats top to bottom; up = they spread out lower down.",
  mullion_tau:"<b>Puts a window in front of the curtain — the grid of bars whose shadow falls across the cloth.</b> 0 = no window. Up = darker, more solid bars. This is the curtain's signature shot: the bars sit centimetres off the cloth, so the sun draws them SHARP, while the tree's leaf-gaps metres away stay soft round sun-images. Same sun, same frame, two different kinds of shadow — and the only thing that decides which is how far the thing casting it stands from the cloth.",
  mullion_pitch_m:"<b>How big the window panes are</b> — the spacing from one bar to the next. A sash window is about 0.3–0.4 m. Small = a many-paned cottage window; large = a few big sheets of glass.",
  mullion_bar_m:"<b>How thick the glazing bars are.</b> Thin = a delicate steel-framed window; thick = chunky painted wood. Just the width of the dark line each bar draws on the cloth.",
  mullion_depth_m:"<b>How far the window stands off the curtain.</b> The knob that makes it a WINDOW and not a print. Two things move together, both real: the grid slides a little (its shadow is thrown by the sun like everything else, just barely, because it's so close), and its edges soften — the further off the cloth, the blurrier, exactly the way the distant leaves are blurry. Push it far enough and the bars go soft too.",
  window_w_m:"<b>How wide the window is — the light stops having an edge and gets one.</b> 0 (either side) = the old behaviour: bright to infinity, bars tiling the whole frame like a fishnet. Give it a width and a height and the glow becomes a real rectangle of sky with dark wall around it, which is most of what makes the shot read as a bedroom at sunset rather than a green field. The frame is the aperture edge itself, and it bends over the pleats exactly like every other shadow does.",
  window_h_m:"<b>How tall the window is.</b> Together with the width it's the on/off switch — either one at 0 means no window at all. A tall narrow aperture reads as a sash or a French door; a wide short one as a kitchen window over the sink. Try it a touch taller than you think: the top edge is usually the one you see.",
  window_cx_m:"<b>Slide the window sideways behind the curtain.</b> Nothing moves on the cloth itself, only the patch of light behind it — so the pleats stay put and the bright rectangle wanders across them.",
  window_cy_m:"<b>Slide the window up and down.</b> This is the sill height, effectively. Low = you're looking at a window you'd sit beside; high = the light comes from over your head and the cloth below is all dim wall.",
  window_wall:"<b>How much light still reaches the cloth outside the window.</b> The wall behind is opaque, so strictly this should be nothing — but the room you're in has its own dim light on this side of the curtain, and without a little of it the surround goes to a flat black void instead of dark cloth you can still read the folds in. A few percent is usually right.",
  tent_ridge_h_m:"<b>How high the flat crown panel sits overhead</b> (receiver <i>tent</i>). This, the half-width below and the shoulder set the wall profile: a high crown over a narrow floor gives steep walls standing up close, a low one over a wide floor gives a flat shelter you can barely sit up in.",
  tent_half_w_m:"<b>How far the side walls reach out where they meet the ground</b> (receiver <i>tent</i>) — half the floor width. Widen it and the skirt lies back; narrow it and it stands up steep and close.",
  tent_crown_w_m:"<b>How wide the flat panel along the top is</b> — half of it, so 0.35 is a 70 cm crown. This is the brow pole's flat, the thing that makes a modern tent feel roomier than an old ridge tent: instead of two slopes pinching to a line above you, there's a real ceiling. At 0 it collapses to a ridge line and you're back in a boy-scout A-frame.",
  tent_shoulder_h_m:"<b>How high the side walls bulge widest.</b> Each side is a smooth arch — a bent pole with fabric stretched over it — and this is where along its climb the arch reaches furthest out. Low puts the belly near the floor and leaves a long lean-in above; high pushes it up under the ceiling. It's held between 30% and 70% of the crown height, outside which the curve would fold back over itself.",
  tent_shoulder_w_m:"<b>How far the side walls bulge out at that height.</b> This is the roundness knob: it's what makes a modern tent feel like a room instead of a wedge, the walls swelling out at elbow height and tucking back in above. Widen it for a fatter, roomier arch. It won't go below the straight line from floor to ceiling — at that line the arch flattens into a plain slope, and narrower than it the wall would curve the wrong way and start cutting the ceiling away.",
  tent_end_apex_h_m:"<b>How high the little triangle at the foot end stands.</b> The far end is not one flat wall: down at the floor there's a small upright triangle — the foot vent — and above it two panels slope up and away to the ceiling, meeting the triangle exactly at its point. This is where that point sits. Low = a squat vent under a lot of sloping roof; high = a tall triangle and short slopes. Push it near the ceiling and the slopes vanish and you're back to one flat end wall.",
  tent_hip_rake:"<b>How steeply the two panels above the foot vent slope away.</b> They have to lean back faster than the end wall does, or there's nothing for them to cut — so this is the extra. Small = a long shallow roof running almost to the end; large = the ceiling stops short and the end is mostly slope. It's what draws the seam down the middle of the ceiling and the two lines that split off it above the vent's point.",
  tent_len_m:"<b>How long the tent is</b> — and, more to the point, where the far end wall stands. You sit 30% of the way down it, so most of this is the space in front of you. Short = the far wall looms close; long = it recedes, but it never disappears, which is the point.",
  tent_end_lean:"<b>How much the end walls lean outward as they drop.</b> 0 = flat vertical walls at each end, like a box. Up = the ends splay out toward the floor the way a real tent's fly does, so the end reads as a leaning plane rather than a painted backdrop.",
  tent_eye_h_m:"<b>How high you're sitting inside the tent.</b> Low = lying down, the crown far overhead and the walls sweeping past you; high = sitting up with your head near the ceiling. It's held just inside every panel — past one you'd be outside your own tent and there'd be nothing to see.",
  tent_fade:"<b>How fast the fabric dims with distance.</b> A gentle depth cue: the far end wall is what actually closes the space, so this just keeps the near fabric a touch brighter than the far, the way real light in a small space falls off. Too much and the far end goes murky.",
  tent_mesh:"<b>Turns the side panels above the widest point into mesh.</b> A real tent isn't one fabric: it's solid nylon up to about elbow height and dark no-see-um mesh above that, and the hard line between them is one of the first things your eye finds from inside. Mesh lets less light through, and — this is the part you actually see — it barely scatters what it does let through, so instead of the soft glow the nylon gives you the dapples come through the band crisp and the band itself reads darker. 0 = one fabric all the way up. The wall doesn't bend at the hem, it just changes material, so the light still rolls smoothly around the curve across it.",
  tent_seam:"<b>The dark taped seams where the panels meet.</b> 0 = none, the whole tent is one continuous sheet. Turn it up and every junction draws — the two long edges of the ceiling running away from you, the seam down the ceiling's centreline at the foot end and the two lines it splits into over the vent, the rims where the ends meet the roof. It's the strongest cue that you're inside a made object of panels and not a smooth painted shell.",
  velvet_sheen:"<b>The velvet glow.</b> The soft pale sheen that catches on the front of each fold — the giveaway of a standing-fibre pile (terciopelo) rather than a flat dyed sheet. 0 = matte; up = a richer velvet shine on the ridges.",
  curtain_scatter:"<b>The soft glow that fills the dark pleat flanks — the velvet lit from within.</b> 0 = only the direct thread-through, so every fold that turns edge-on goes black. Light gets through backlit pile two ways: some of it threads the fibres straight (very picky about angle, which is why the edge-on flanks kill it), and the rest bounces around inside the pile and comes out diffused, spreading almost every way at once — so a fold's angle barely dims it. This knob is the share that takes the second route. It doesn't add light, it splits the light already coming through, so the cloth can never end up brighter than what falls on it.",
  curtain_diffuse:"<b>The glow that bleeds ACROSS a shadow edge — the haze the pile-scatter knob can't make.</b> Light doesn't only pass straight through cloth, it also travels sideways inside the weave, so a hot patch spills a little of itself into the dark right beside it. That's the difference between a light patch that reads as a hard spotlight and one that reads as a soft glowing haze — and the reference photo has both in the same frame. 0 = off and nothing changes. Up = the bright patches grow halos and the shade next to them lifts. It splits the light rather than adding any, so the picture never gets brighter overall, just softer where it's hot. The expensive one: it's the only effect here that has to look at neighbouring pixels, so it costs real frame time — check the profiler.",
  curtain_diffuse_m:"<b>How far the weave carries the light sideways.</b> Measured in centimetres of actual cloth, not pixels, so zooming the camera doesn't change the physics — a thin tight nylon carries it a couple of centimetres, a thick loose weave further. Small = a tight halo hugging each bright patch; large = a broad wash that softens the whole cast. Very large values stop growing (the blur only has so many samples to spend), so if pushing it does nothing you've hit that ceiling.",
