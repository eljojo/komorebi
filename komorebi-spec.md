...
- **Twigs** — every clump carries its own faster, stiffer bend angle, and swings about a **stem joint** — a base point offset from the clump *toward its limb* (with a little per-twig angular spread so they don't lockstep). Bending the twig sweeps the whole clump to one side through an arc; leaves sweep arcs and the clump turns, decorrelated clump-to-clump.
