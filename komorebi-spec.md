...
- **One shared engine, two consumers.** The whole renderer is a standalone module — `komorebi.js`, exposing `Komorebi.create(canvas, { params })` → a handle (`apply(scope)`, `setParams`, `transitionTo(params, {duration, onEnd})` for a cloud-bloom crossfade between looks (§9), live `params`/`perf`/`motion`/`src`/`trans`, an `onFrame` hook). It runs its own rAF loop on whatever canvas it's handed and carries no UI. Two pages consume it: the **editor** (`editor.html` — the dev panel, HUD, source inset, preset management, sun-drag) and a **player** (`player.html` — a minimal viewer-only reference: a full-bleed canvas cycling through presets, no UI; the eljojo.net homepage embeds the same pattern). `create()` throws on missing WebGL2/float targets so the player can degrade to a static page.
