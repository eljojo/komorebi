...
- **Limbs** — a handful of plan anchors; each carries a **scalar bend angle** on a soft/slow spring. A limb's bend **swings its whole clump as a rigid arc about the limb's anchor** — so the clump both moves *and rotates*, and clumps sharing a limb swing together. The bend follows the coherent gust (limbs lean downwind together) plus a slow spatial drift, so limbs differ.
- **Twigs** — every clump carries its own faster, stiffer bend angle that **spins the clump about its own centre**: leaves sweep small arcs and the clump's internal pattern *rotates* (gaps reorganize), decorrelated clump-to-clump.

A leaf inherits **twig-spin ∘ limb-swing ∘ trunk-translation**, and — critically — its **own orientation rotates with the branch** (`leaf angle += limb bend + twig bend`), which is what makes its footprint present differently as the branch turns. Because each clump is now a *rigid pivot* rather than a free-floating offset, the motion reads as branches in air instead of the earlier soup. The bend **breathes with the gust** and **rests at zero wind** (still image untouched). Still **shallow and abstracted** — limbs are plan-only anchors with no grown 3-D arms, and a clump pivots about its own centroid rather than a true stem joint — but the principle (*wind changes the angles of branches; children inherit the rotation*) is now the spec's, and it is the single change that moved the result out of the uncanny valley.

> Lesson, restated and stronger: not only does moving *positions* alone look like a slide (above) — moving positions *differently per region* looks like **soup**. The differential motion of a canopy is **rotational**: angles changing at joints, inherited down a tree. Translation is only the trunk's bulk drift; everything below it is rotation.
