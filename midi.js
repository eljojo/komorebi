...
export function createMidi({ setNorm, getNorm, onBind }) {
  ...
  // Soft takeover: while a binding hasn't been captured, hold the slider until the knob
  // crosses its current value (the side flips), then track every message. `side` is the
  // knob's position relative to the slider, seeded at bind time.
  function applyValue(b, n) {
    if (!b.captured) {
      const cur = getNorm(b.key);
      const side = Math.sign(n - cur);                           // knob vs. slider
      if (side === 0) { b.captured = true; b.side = undefined; }       // exactly at value → grab
      else if (b.side === undefined) { b.side = side; return; }        // first sample → record, wait
      else if (side === b.side) return;                               // same side → keep waiting
      else { b.captured = true; b.side = undefined; }                 // crossed → grab it
    }
    setNorm(b.key, n);
  }
  ...
  function ingest(ev) {                  // ev = { channel, cc, value }
    ...
    if (armedKey !== null) {
      ...
      const side = Math.sign(ev.value / 127 - getNorm(armedKey));   // seed takeover from the knob's current spot
      bindings.set(id, { key: armedKey, label, captured: side === 0, side: side === 0 ? undefined : side });
  ...
  return {
    start, feed, bound, unbind,
    arm: (key) => { armedKey = key; },
    disarm: () => { armedKey = null; },
    recapture: (key) => {
      for (const b of bindings.values()) if (b.key === key) { b.captured = false; b.side = undefined; }
    },
